...
export class WhatsappService {
  ...
  async handleIncomingMessage(body: Record<string, unknown>): Promise<{ status: string }> {
    try {
      await this.processWebhook(body as WhatsAppMessagePayload);
      return { status: 'ok' };
    } catch (error) {
  ...
  async listConversations(params: { page: number; limit: number; status?: string }): Promise<{ data: unknown[]; total: number; page: number; limit: number }> {
  ...
  async updateConversation(id: string, body: Record<string, unknown>): Promise<unknown> {
    const { conversationState, role, preferredLanguage, isHumanHandoff, handoffReason, tags, notes } = body as {
      conversationState?: string;
      role?: string;
      preferredLanguage?: string;
    ...
    return this.prisma.whatsAppConversation.update({
      where: { id },
      data: {
        ...(conversationState && { conversationState: conversationState as WhatsAppConversationState }),
  ...
  async sendMessageToWhatsApp(body: { to: string; message: string; templateName?: string }): Promise<SendResult> {
    const config = await this.prisma.whatsAppConfig.findFirst({ where: { isActive: true } });
    if (!config) {
      return { success: false, error: 'No active WhatsApp config found', errorType: ErrorType.CONFIG_MISSING };
    }
  ...
  async getStats(params: { from?: string; to?: string }): Promise<{
  ...
  getErrorStatsWithFilters(params: { from?: string; to?: string; limit?: number }): { errors: ErrorRecord[]; stats: Record<string, number> } {
