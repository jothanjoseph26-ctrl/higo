...
type Props = NativeStackScreenProps<DriverMainStackParamList, 'TripChat'>;
...
export function TripChat({ route }: Props) {
  ...
  const driverId = useDriverAuthStore((state) => state.driver?.id);
  ...
  const mapMessages = useCallback(
    (items: TripMessage[]): ChatRow[] =>
      items.map((message) => ({
        ...message,
        isMine: message.senderId === driverId,
      })),
    [driverId],
  );
    ...
    const onMessageNew = (payload: TripMessageNewPayload) => {
      ...
      setMessages((prev) => {
        ...
        return [
          ...
          {
            ...payload.message,
            isMine: payload.message.senderId === driverId,
          },
        ];
      });
    };
