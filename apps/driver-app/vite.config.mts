...
const rollupPlugin = (matchers: RegExp[]) => ({
  ...
  async load(id: string) {
    ...
    console.log(`[js-in-jsx] matched: ${normalizedId}`);
    ...
    const loader =
      ...
      : null;
    console.log(`[js-in-jsx] loader: ${loader} for ${cleanId}`);
    if (!loader) return;
    const file = readFileSync(id.split('?')[0].split('#')[0], { encoding: 'utf-8' });
    try {
      const res = await transformWithEsbuild(file, cleanId, { loader, jsx: 'automatic' });
      console.log(`[js-in-jsx] transform success for ${cleanId}`);
      return res;
    } catch (e) {
